...
class MongoDBService<T extends mongoose.Document> {
  ...
  private handleResponse(status: boolean, data: any = null, message = '', error: any = null): DbResponse {
  ...
  async create(data: Partial<T>, options?: DbOptions): Promise<DbResponse<T>> {
    ...
    try {
      ...
      return this.handleResponse(true, savedDoc, 'Document created successfully');
    } catch (error) {
      if (session) await this.abortTransaction();
      return this.handleResponse(false, null, 'Failed to create document', error);
    }
  }

  async findById(id: mongoose.Types.ObjectId, options?: DbOptions): Promise<DbResponse<T>> {
    const session = options?.session || this.session || null;
    try {
      const doc = await this.model.findById(id).session(session).lean().exec();
      return doc ? this.handleResponse(true, doc) : this.handleResponse(false, null, 'Document not found');
    } catch (error) {
      return this.handleResponse(false, null, 'Failed to retrieve document', error);
    }
  }
  ...
  async findOne(query: any, options?: DbOptions): Promise<DbResponse<T>> {
    const session = options?.session || this.session || null;
    try {
      let queryBuilder = this.model.findOne(query).session(session) as mongoose.Query<T | null, T>;
      if (options?.select) {
        queryBuilder = queryBuilder.select(options.select) as mongoose.Query<T | null, T>;
      }
  ...
  async find(query: any = {}, limit = 10, skip = 0, options?: DbOptions): Promise<DbResponse<T[]>> {
    const session = options?.session || this.session || null;
    try {
      const docs = await this.model.find(query).limit(limit).skip(skip).session(session).exec();
      return this.handleResponse(true, docs, 'Documents retrieved successfully');
    } catch (error) {
      return this.handleResponse(false, [], 'Failed to retrieve documents', error);
    }
  }

  async updateOne(query: any, updateData: Partial<T>, options?: DbOptions): Promise<DbResponse<T>> {
    const session = options?.session || this.session || null;
    try {
      ...
      if (session) await this.abortTransaction();
      return this.handleResponse(false, null, 'Failed to update document', error);
    }
  }
  ...
  async deleteOne(query: any, options?: DbOptions): Promise<DbResponse<T>> {
    const session = options?.session || this.session || null;
    try {
      const deletedDoc = await this.model.findOneAndDelete(query).session(session).exec();
      return deletedDoc
      ...
      if (session) await this.abortTransaction();
      return this.handleResponse(false, null, 'Failed to delete document', error);
    }
  }
  ...
  async deleteMany(query: any, options?: DbOptions): Promise<DbResponse<T>> {
    ...
    try {
      ...
      return result.deletedCount > 0
        ? this.handleResponse(true, result, `${result.deletedCount} documents deleted successfully`)
        : this.handleResponse(false, null, 'No documents found to delete');
    } catch (error) {
      if (session) await this.abortTransaction();
      return this.handleResponse(false, null, 'Failed to delete documents', error);
    }
  }
}
